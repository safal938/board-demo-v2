...
const AdverseEventAnalytics: React.FC<AdverseEventAnalyticsProps> = ({ patientData }) => {
  const latestEncounter = patientData?.encounters?.[patientData.encounters.length - 1];
  const analysisData = latestEncounter?.rucam_ctcae_analysis;

  if (!analysisData) {
    return (
      <AnalyticsContainer>
        <SectionTitle>Adverse Event Analytics</SectionTitle>
        <SectionDescription>No adverse event analysis data available.</SectionDescription>
      </AnalyticsContainer>
    );
  }

  const { rucam, ctcae, reasoning } = analysisData;

  // Mock adverse events data based on CTCAE
  const adverseEvents = [
    { event: 'Hepatotoxicity', severity: 'Severe', description: 'ALT/AST >20× ULN with jaundice' },
    { event: 'Encephalopathy', severity: 'Severe', description: 'Confusion, asterixis, disorientation' },
    { event: 'Coagulopathy', severity: 'Severe', description: 'Elevated INR, impaired synthesis' }
  ];
  ...
  return (
    <AnalyticsContainer>
      ...
      <AdverseEventsSection id="adverse-events-zone">
        ...
        <SectionDescription>
          Brief description about the presenting complaints or symptoms after admission.
          Severity mapping based on 4 level scoring system (mild, moderate, severe, critical) or CTCAE
        </SectionDescription>
        ...
        <TableContainer>
          <Table>
            ...
            <TableRow>
              <TableCell style={{ fontWeight: '600', background: '#f8fafc' }}>Event</TableCell>
              <TableCell style={{ fontWeight: '600', background: '#f8fafc' }}>Severity</TableCell>
              <TableCell style={{ fontWeight: '600', background: '#f8fafc' }}>Description</TableCell>
            </TableRow>
      ...
      <CausalitySection id="causality-assessment-zone">
        ...
        <SectionDescription>Auto-calculated RUCAM like scoring system</SectionDescription>
        
        <RucamWidget>
          <div style={{ fontWeight: '600', marginBottom: '8px' }}>Causality Assessment Widget</div>
      ...
      <ReasoningSection id="medical-reasoning-zone">
        ...
        <SectionDescription>
          ...
          Evidence (Eg: ALT rise 6 days post exposure)
          Rationale: Pattern, timing, dechallenge
        </SectionDescription>
        
        <HighlightBox>
          <HighlightText>
            <strong>Assessment:</strong> {rucam.causality_category} DILI • <strong>CTCAE Grade:</strong> {ctcae.overall_grade}
          </HighlightText>
        </HighlightBox>
