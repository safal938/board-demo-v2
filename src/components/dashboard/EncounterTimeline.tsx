...
const EncounterTimeline: React.FC<EncounterTimelineProps> = ({
  encounters,
  medicationTimeline,
}) => {
  const getConfidenceScore = (encounter: any): number => {
    // Calculate AE risk based on actual data from JSON
    const riskColor = encounter.meta?.ui_risk_color;
    const hasFlags = encounter.meta?.ui_flags?.length > 0;
    const hasAdverseEvents = encounter.meta?.event_tags?.length > 0;
    const hasSafetyFlags =
      encounter.safety_flags?.dili_suspected ||
      encounter.safety_flags?.hy_law_status;

    // Use actual risk indicators from the data
    if (
      hasSafetyFlags &&
      encounter.safety_flags?.hy_law_status === "suspected_red"
    ) {
      return 85; // High risk - Hy's Law suspected
    }

    if (hasAdverseEvents) {
      const adverseEventCount = encounter.meta.event_tags.length;
      return Math.min(75 + adverseEventCount * 5, 95); // 75-95% based on number of adverse events
    }

    if (riskColor === "red") {
      return 80; // High risk based on UI risk color
    }

    if (hasFlags || riskColor === "amber") {
      const flagCount = encounter.meta?.ui_flags?.length || 1;
      return 45 + flagCount * 10; // 45-65% based on number of flags
    }

    if (riskColor === "green") {
      return 15; // Low risk - green encounters
    }

    // Default for encounters without risk data
    return 25;
  };

  const getMedicationChangesForEncounter = (encounter: any) => {
    const encounterDate = new Date(encounter.meta?.date_time);
    ...
    // Add changes from encounter plan if available
    if (encounter.plan?.medications_started) {
      encounter.plan.medications_started.forEach((med: any) => {
        changes.push({
          type: "start",
          medication: `${med.name} ${med.dose}`,
          frequency: med.frequency,
  ...
  const renderEncounterCard = (encounter: any, index: number) => {
    ...
    return (
      <EncounterCard key={encounter.encounter_no || index}>
        ...
        <EncounterDate>{formatDate(encounter.meta?.date_time)}</EncounterDate>
        ...
        {encounter.meta?.ui_flags && encounter.meta.ui_flags.length > 0 && (
          <InfoSection>
            <InfoLabel>Notes:</InfoLabel>
            <InfoValue>{encounter.meta.ui_flags[0]}</InfoValue>
          </InfoSection>
        )}
      </EncounterCard>
    );
  ...
  const createMedicationLines = () => {
    ...
    const encounterDates = encounters.map(
      (enc) => new Date(enc.meta?.date_time)
    );
    ...
    medicationTimeline.forEach((med) => {
      ...
      const startDate = new Date(med.start);
      const endDate = med.end ? new Date(med.end) : new Date();
